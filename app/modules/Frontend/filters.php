...
Route::filter('frontend.action',function($route,$request){
    ...
    Queue::push('\Forret\Repositories\ActionRepository@frontendQuery',array('action'=>$action,'uri'=>Request::path(),'method'=>Request::method(),'description'=>$description,'messages'=>$messages,'payload'=>$payload,'user_id'=>$userId,'session'=>Session::GetId()));
});
